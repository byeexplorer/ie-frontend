...
function Hamburger({ isMenuOpen, onClick }: Props) {
  return (
    <div
      ...
      className="flex flex-col justify-center align-center z-20 translate-y-[0.1rem]"
      onClick={onClick}
    >
      <div id="hamburger" className="flex flex-col justify-around align-center h-[0.8rem]">
        <div
          className={`h-[0.05rem] w-[0.75rem] transition-all bg-white rounded origin-top-left ${
            isMenuOpen ? 'rotate-45' : 'none'
          }`}
        />
        <div
          className={`h-[0.05rem] w-[0.75rem] transition-all bg-white rounded ${
